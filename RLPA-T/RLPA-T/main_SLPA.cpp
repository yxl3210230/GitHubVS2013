...
int main(int argc, char* argv[]) {
	...
	//			SLPA
	//----------------------------------------
	//if(inputFileName.size()==0 || !isFileExist(inputFileName.c_str())){	//这里输入的是目录，不用在判断文件是否存在
	//	cout<<"ERROR: input network "+inputFileName+" not found!"<<endl;
	//	exit(1);
	//}else{

	vector<string> inputfiles;	//文件下的文件名

	FindFiles(inputfiles, ".\\" + inputFileName);
	...
	cout << "Find " << inputfiles.size() << " files in floder " + inputFileName + " ." << endl;
	if (inputfiles.size() == 0){
		...
		SLPA slpa(inputfiles, THRS, maxRun, maxT, outputDir, isUseLargestComp, numThreads);
